...
#amending corner point
ymc2_a=-100000
#split A: Svalbard
mask1_a = (xp < xmc1+dd) * (xp > xmc1) * (yp>ymc1) * (yp<ymc2_a)
#south
mask2_a = (xp < xmc2+dd) * (xp > xmc2) * (yp>ymc1) * (yp<ymc2_a)
#east
mask3_a = (xp < xmc2) * (xp > xmc1) * (yp>ymc2_a) * (yp<ymc2_a+dd)
#west
mask4_a = (xp < xmc2) * (xp > xmc1) * (yp>ymc1) * (yp<ymc1+dd)

#split b: FJL
mask1_b = (xp < xmc1+dd) * (xp > xmc1) * (yp>ymc2_a) * (yp<ymc2)
#south
mask2_b = (xp < xmc2+dd) * (xp > xmc2) * (yp>ymc2_a) * (yp<ymc2)
#east
mask3_b = (xp < xmc2) * (xp > xmc1) * (yp>ymc2) * (yp<ymc2+dd)
#west
mask4_b = (xp < xmc2) * (xp > xmc1) * (yp>ymc2_a) * (yp<ymc2_a+dd)

#amending corner points
ymc2_c=1100000
xmc1_c=400000
xmc2_c=800000
#split c: Severnaya Zemlya
mask1_c = (xp < xmc1_c+dd) * (xp > xmc1_c) * (yp>ymc2) * (yp<ymc2_c)
#south
mask2_c = (xp < xmc2_c+dd) * (xp > xmc2_c) * (yp>ymc2) * (yp<ymc2_c)
#east
mask3_c = (xp < xmc2_c) * (xp > xmc1_c) * (yp>ymc2_c) * (yp<ymc2_c+dd)
#west
mask4_c = (xp < xmc2_c) * (xp > xmc1_c) * (yp>ymc2) * (yp<ymc2+dd)

#make mask for the areal average in the box
mask5 = (xp < xmc2) * (xp > xmc1) * (yp>ymc1) * (yp<ymc2)
mask5_a = (xp < xmc2) * (xp > xmc1) * (yp>ymc1) * (yp<ymc2_a)
mask5_b = (xp < xmc2) * (xp > xmc1) * (yp>ymc2_a) * (yp<ymc2)
mask5_c = (xp < xmc2_c) * (xp > xmc1_c) * (yp>ymc2) * (yp<ymc2_c)
...
#A
c3lon_a,c3lat_a = transform(osi_proj,lonlat,ymc2_a,-xmc2)
c4lon_a,c4lat_a = transform(osi_proj,lonlat,ymc2_a,-xmc1)

#C
c1lon_c,c1lat_c = transform(osi_proj,lonlat,ymc2,-xmc1_c)
c2lon_c,c2lat_c = transform(osi_proj,lonlat,ymc2,-xmc2_c)
c3lon_c,c3lat_c = transform(osi_proj,lonlat,ymc2_c,-xmc2_c)
c4lon_c,c4lat_c = transform(osi_proj,lonlat,ymc2_c,-xmc1_c)



#print 'lons: ',c4lon,c3lon,c3lon_c,c4lon_c
#print 'lats: ',c4lat,c3lat,c3lat_c,c4lat_c
#exit()
...
for fn in iclist:
    ...
    #C
    u_n_c = np.ma.array(u,mask=~mask1_c)
    u_s_c = np.ma.array(u,mask=~mask2_c)
    v_e_c = np.ma.array(v,mask=~mask3_c)
    v_w_c = np.ma.array(v,mask=~mask4_c)
    ...
    #C
    flux_n_c = u_n_c * icecon
    flux_s_c = u_s_c * icecon
    flux_e_c = v_e_c * icecon
    flux_w_c = v_w_c * icecon
    ...
    #C
    flux_c = np.sum(flux_n_c) - np.sum(flux_s_c) - np.sum(flux_e_c) + np.sum(flux_w_c)
    if (flux_s_c.all() is np.ma.masked): flux = -np.sum(flux_n_c) - np.sum(flux_e_c) + np.sum(flux_w_c)
    flux_c = flux_c/100
    ...
    #C
    icecon_subset_c = np.ma.array(icecon,mask=~mask5_c)
    m_icecon_c.append(np.mean(icecon_subset_c))
    ...
    #m.drawmeridians(np.arange(0.,180.,20.),latmax=85.,labels=[0,0,0,1], fontsize=16)
    ...
    ##B
    #lonbox = [c4lon_a,c3lon_a,c3lon,c4lon]
    #latbox = [c4lat_a,c3lat_a,c3lat,c4lat]
    #xbox,ybox = m(lonbox,latbox)
    #xybox = zip(xbox,ybox)
    #poly = Polygon(xybox, edgecolor='w', alpha=1, fill=False, linewidth=3, linestyle='-')
    #plt.gca().add_patch(poly)
    ...
    ##C
    #lonbox = [c1lon_c,c2lon_c,c3lon_c,c4lon_c]
    #latbox = [c1lat_c,c2lat_c,c3lat_c,c4lat_c]
    #xbox,ybox = m(lonbox,latbox)
    #xybox = zip(xbox,ybox)
    #poly = Polygon(xybox, edgecolor='w', alpha=1, fill=False, linewidth=3, linestyle='-')
    #plt.gca().add_patch(poly)
...
#np.save(inpath+'advection_c',advection_c)

#np.save(inpath+'m_icecon_a',m_icecon_a)
#np.save(inpath+'m_icecon_b',m_icecon_b)
#np.save(inpath+'m_icecon_c',m_icecon_c)
...
advection_c = np.load(inpath+'advection_c.npy')

m_icecon_a = np.load(inpath+'m_icecon_a.npy')
m_icecon_b = np.load(inpath+'m_icecon_b.npy')
m_icecon_c = np.load(inpath+'m_icecon_c.npy')

dates = np.load(inpath+'date_flux.npy')

#make time series, monthly means and standard deviation
import pandas as pd
...
ax.plot(dates,advection_a*24*60*60/1e3, c='orange', marker='.', linestyle='None', ms = 3, alpha = .5)
ax.plot(dates,advection_b*24*60*60/1e3, c='g', marker='.', linestyle='None', ms = 3, alpha = .5)
ax.plot(dates,advection_c*24*60*60/1e3, c='purple', marker='.', linestyle='None', ms = 3, alpha = .5)
...
area = (xmc1-xmc2)*(ymc1-ymc2_a)                    #in m^2
...
#area = (xmc1-xmc2)*(ymc2_a-ymc2)                    #in m^2
...
bx.plot(dates,m_icecon_a, c='orange', label='WB', marker='.', linestyle='None', ms = 3, alpha = .5)
bx.plot(dates,m_icecon_b, c='g', label='FJL', marker='.', linestyle='None', ms = 3, alpha = .5)
bx.plot(dates,m_icecon_c, c='purple', label='SZ', marker='.', linestyle='None', ms = 3, alpha = .5)
...
cx.plot(dates,ic_area_melt/1e3, c='orange', marker='.', linestyle='None', ms = 3, alpha = .5)

#b
area = (xmc1-xmc2)*(ymc2_a-ymc2)
ic_area_all = area/1e6*m_icecon_b
ic_area_change = np.zeros_like(ic_area_all)
ic_area_change[1:] = ic_area_all[1:]- ic_area_all[:-1]      #if next time step is greater=positive value=increase=sea ice growth in leads&import
ic_area_export = advection_b *60*60*24                      #km^2/s to km^2/day
ic_area_melt = ic_area_change + ic_area_export              #km^2/day               import=increase=positive value

cx.plot(dates,ic_area_melt/1e3, c='g', marker='.', linestyle='None', ms = 3, alpha = .5)

#c
area = (xmc1_c-xmc2_c)*(ymc2-ymc2_c)
ic_area_all = area/1e6*m_icecon_c
ic_area_change = np.zeros_like(ic_area_all)
ic_area_change[1:] = ic_area_all[1:]- ic_area_all[:-1]      #if next time step is greater=positive value=increase=sea ice growth in leads&import
ic_area_export = advection_c *60*60*24                      #km^2/s to km^2/day
ic_area_melt = ic_area_change + ic_area_export              #km^2/day               import=increase=positive value

cx.plot(dates,ic_area_melt/1e3, c='purple', marker='.', linestyle='None', ms = 3, alpha = .5)
...
mons = mdates.MonthLocator()
yrs = mdates.YearLocator()

#ax.xaxis.set_major_locator(mons)
#ax.xaxis.set_major_formatter(mdates.DateFormatter('%d-%b'))
ax.xaxis.set_minor_locator(mons)

#bx.xaxis.set_major_locator(mons)
#bx.xaxis.set_major_formatter(mdates.DateFormatter('%d-%b'))
bx.xaxis.set_minor_locator(mons)

cx.xaxis.set_major_locator(yrs)
cx.xaxis.set_major_formatter(mdates.DateFormatter('%b-%y'))
cx.xaxis.set_minor_locator(mons)
...
bx.legend(loc='lower left',prop={'size':10}, fancybox=True, framealpha=.5, ncol=3)
fig2.tight_layout()
fig2.savefig(outpath+'advection_ts_long')
...
bx = fig3.add_subplot(132)
bx.set_title('FJL')
bx.scatter(advection_b[:-2],m_icecon_b[2:],marker='o')

bx = fig3.add_subplot(133)
bx.set_title('SZ')
bx.scatter(advection_c[:-2],m_icecon_c[2:],marker='o')


fig3.savefig(outpath+'advection_cc_long')